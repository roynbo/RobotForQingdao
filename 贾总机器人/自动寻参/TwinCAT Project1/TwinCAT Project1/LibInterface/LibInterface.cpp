...
HRESULT CLibInterface::CycleUpdate(ITcTask* ipTask, ITcUnknown* ipCaller, ULONG_PTR context)
{
	...
	if (sign == 0)
	{
		if (veloX != 0 || veloY != 0)
		{
			robot.CalculateHandVelo(veloX, veloY, instruct);
		}
		else
		{
			robot.HandStop();
...
)
{
	...
	case LibInterfaceIndexGroup1:
		...
		case LibInterfaceIndexOffset5://x位置
			...
			b = b / 180 * PI;
			c = c / 180 * PI;
			tempx = 0.5*cos_(a) + 0.44*cos_(b) + 0.3*cos_(c);
			pData = &tempx; }
			AdsReadWriteRes(rAddr, invokeId, ADSERR_NOERR, sizeof(double), pData);
			break;
		...
		case LibInterfaceIndexOffset6://y位置
			...
			b = b / 180 * PI;
			c = c / 180 * PI;
			tempy = 0.5*sin_(a) + 0.44*sin_(b) + 0.3*sin_(c);
...
void CLibInterface::ReadFromPLC()
{
	...
	robot.arm.SwingArm[2].AxisStatus.position = m_Inputs.SwingArm3Output.Position;
	...
	robot.arm.SwingArm[3].AxisStatus.position = m_Inputs.SwingArm4Output.Position;
	...
	robot.mainpulator.Mainpu[6].AxisStatus.Current = m_Inputs.ClampOutput.Current;
}
...
void CLibInterface::WriteToPLC()
{
	...
	m_Outputs.ClampInput.Velocity = robot.mainpulator.Mainpu[6].AxisCommand.Velocity/-10;
