...
HRESULT CLibInterface::CycleUpdate(ITcTask* ipTask, ITcUnknown* ipCaller, ULONG_PTR context)
{
	...
	if (sign == 0)
	{
		if (handVelX != 0 || handVelY != 0 || handVelZ != 0)
		{
			robot.CalculateHandVelo(handVelX, handVelY, handVelZ, instruct);
		}
		else
		{
			robot.HandStop();
	...
	robot.DecoderInstruct(instruct, indexCrl, velo, pos, carVeloX, carVeloY, handVelX, handVelY, handVelZ,flag);     //解码函数2，命令落实
	if (instruct != 0)
	{
		tpp = 1;
	}
...
)
{
	...
	case LibInterfaceIndexGroup1:
		...
		case LibInterfaceIndexOffset5://x位置
			...
			double d = robot.mainpulator.Mainpu[0].AxisStatus.position / 27.83 * 90;
			double tempa = a;
			double tempb = b;
			a = a + 90;
			b = b + tempa + 90;
		...
		case LibInterfaceIndexOffset9://小臂位置
			...
			AdsReadWriteRes(rAddr, invokeId, ADSERR_NOERR, sizeof(double), pData);
			break;
		case LibInterfaceIndexOffset10://腰位置
			m_Trace.Log(tlInfo, FNAMEA "oid=0x%08x indexGroup=0x%08x, indexOffset=0x%08x",
				m_objId.value, indexGroup, indexOffset);
			{
				double a = robot.mainpulator.Mainpu[0].AxisStatus.position/27.83*90;   //修改腰的角度对应数值
				pData = &a; }
			AdsReadWriteRes(rAddr, invokeId, ADSERR_NOERR, sizeof(double), pData);
			break;
		case LibInterfaceIndexOffset11://转动机械臂位置
			m_Trace.Log(tlInfo, FNAMEA "oid=0x%08x indexGroup=0x%08x, indexOffset=0x%08x",
				m_objId.value, indexGroup, indexOffset);
			{
				//double a = ((robot.mainpulator.Mainpu[5].AxisStatus.position-481.5)/ 4738.5 *90+9.11)/1.69*90;
				double a = robot.mainpulator.Mainpu[5].AxisStatus.position*1.0118 - 1.88;
				pData = &a; }
			AdsReadWriteRes(rAddr, invokeId, ADSERR_NOERR, sizeof(double), pData);
			break;
		case LibInterfaceIndexOffset12://z位置
			m_Trace.Log(tlInfo, FNAMEA "oid=0x%08x indexGroup=0x%08x, indexOffset=0x%08x",
				m_objId.value, indexGroup, indexOffset);
			{double tempx = 0;
			double a = robot.mainpulator.Mainpu[1].AxisStatus.position;
	...
	case LibInterfaceIndexGroup2:
		...
		case LibInterfaceIndexOffset10://读取手Z速度
		{	m_Trace.Log(tlInfo, FNAMEA "oid=0x%08x indexGroup=0x%08x, indexOffset=0x%08x",
			m_objId.value, indexGroup, indexOffset);
		AdsReadWriteRes(rAddr, invokeId, ADSERR_NOERR, sizeof(double), pData);
		double *pCounter10 = (double *)pData;
		handVelZ = *pCounter10; }
		break;
		}
		break;
